...
export abstract class BaseAgent {
  ...
  async execute(input: BaseAgentInput): Promise<BaseAgentOutput> {
    ...
    logger.info(`Agent ${this.agentId} starting execution`, {
    ...
    try {
      ...
      logger.info(`Agent ${this.agentId} completed execution`, {
        agentId: this.agentId,
        executionTime,
      ...
      logger.error(`Agent ${this.agentId} execution failed`, {
        agentId: this.agentId,
        executionTime,
        error
      });
  ...
  protected validateInput(input: BaseAgentInput): BaseAgentInput {
    try {
      ...
      logger.error(`Input validation failed for agent ${this.agentId}`, { error });
  ...
  protected async validate(
    ...
    try {
      ...
      logger.error(`Output validation failed for agent ${this.agentId}`, { error });
  ...
  protected generateNextSteps(decisions: ArchitecturalDecision[]): string[] {
    ...
    const highImpactDecisions = decisions.filter(d => 
      d.consequences.some(c => c.toLowerCase().includes('critical') || c.toLowerCase().includes('major'))
    );
